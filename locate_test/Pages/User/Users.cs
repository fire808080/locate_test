    ...
    public partial class Users : UserControl
    {
        ...
        private void radioButton4_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    List<LTS.User> user = new List<LTS.User>();
                    user = DAT.DataAccess.GetUser().Where(s => s.UserActivated == false).ToList();
                    for (int i = 0; i < user.Count; i++)
                    {
                        dataGridView1.Rows.Add(user[i].UserID, user[i].UserIdentityNumber, user[i].UserName, user[i].UserSurname,
                            user[i].UserEmail, user[i].UserAdmin, user[i].UserActivated);
        ...
        private void saveFileDialog1_FileOk(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                Paragraph writing = new iTextSharp.text.Paragraph("Rfid Reader Management System " + Environment.NewLine + "Users Information: " + what + "                " + DateTime.Now.ToString() + Environment.NewLine + Environment.NewLine);
